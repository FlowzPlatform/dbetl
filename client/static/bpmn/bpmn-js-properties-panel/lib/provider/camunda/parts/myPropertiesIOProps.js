...
module.exports = function(group, element, bpmnFactory, translate, options) {
  ...
  if (!bo.sourceRef.extensionElements || !bo.targetRef.extensionElements) {
    ...
    _source = _.chain(bo.sourceRef.extensionElements.values).find((d) => { return d.$type == 'camunda:MyProperty' }).result('fields').filter((d) => { return d.entityschema }).value()
  ...
  group.entries.push(mappingSelectBox({
    ...
    selectOptions: function(element, inputNode) {
      ...
      if (formField && formField.consumer && formField.producer) {
        var schemamapping = _.filter(options.schemamapping, function(d) { return d.consumer == formField.consumer && d.producer == formField.producer })
